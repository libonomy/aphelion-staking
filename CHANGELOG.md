...
- [privval] [\#4275](https://github.com/evdatsion/aphelion-dpos-bft/issues/4275) Fix consensus failure when remote signer drops (@melekes)
...
- [blockchain] [\#2699](https://github.com/evdatsion/aphelion-dpos-bft/issues/2699) Update the maxHeight when a peer is removed
...
- Apps
  ...
  - [abci] [\#3403](https://github.com/evdatsion/aphelion-dpos-bft/issues/3403) Remove `time_iota_ms` from BlockParams. This is a
...
- [libs/db] [\#3378](https://github.com/evdatsion/aphelion-dpos-bft/issues/3378) CLevelDB#Stats now returns the following properties:
  ...
  - leveldb.alivesnaps
  - leveldb.aliveiters
- [privval] [\#3351](https://github.com/evdatsion/aphelion-dpos-bft/pull/3351) First part of larger refactoring that clarifies and separates concerns in the privval package.
...
- Blockchain Protocol
  ...
  - [types] [\#2609](https://github.com/evdatsion/aphelion-dpos-bft/issues/2609) ConsensusParams.Hash() is the hash of the amino encoded
...
- [\#2711](https://github.com/evdatsion/aphelion-dpos-bft/issues/2711) Validate all incoming reactor messages. Fixes various bugs due to negative ints.
...
- Blockchain Protocol
  ...
  - [types] Updates to the block Header:
    - [\#1815](https://github.com/evdatsion/aphelion-dpos-bft/issues/1815) NextValidatorsHash - hash of the validator set for the next block,
      ...
      validators
    - [\#2106](https://github.com/evdatsion/aphelion-dpos-bft/issues/2106) ProposerAddress - address of the block's original proposer
  - [consensus] [\#2203](https://github.com/evdatsion/aphelion-dpos-bft/issues/2203) Implement BFT time
    - Timestamp in block must be monotonic and equal the median of timestamps in block's LastCommit
  - [crypto] [\#2239](https://github.com/evdatsion/aphelion-dpos-bft/issues/2239) Secp256k1 signature changes (See
    [ADR-014](https://github.com/evdatsion/aphelion-dpos-bft/blob/develop/docs/architecture/adr-014-secp-malleability.md)):
    - format changed from DER to `r || s`, both little endian encoded as 32 bytes.
    - malleability removed by requiring `s` to be in canonical form.

- P2P Protocol
  - [p2p] [\#2263](https://github.com/evdatsion/aphelion-dpos-bft/issues/2263) Update secret connection to use a little endian encoded nonce
  - [blockchain] [\#2213](https://github.com/evdatsion/aphelion-dpos-bft/issues/2213) Fix Amino routes for blockchain reactor messages
    (@peerlink)

FEATURES:

- [types] [\#2015](https://github.com/evdatsion/aphelion-dpos-bft/issues/2015) Allow genesis file to have 0 validators (@b00f)
  - Initial validator set can be determined by the app in ResponseInitChain
- [rpc] [\#2161](https://github.com/evdatsion/aphelion-dpos-bft/issues/2161) New event `ValidatorSetUpdates` for when the validator set changes
- [crypto/multisig] [\#2164](https://github.com/evdatsion/aphelion-dpos-bft/issues/2164) Introduce multisig pubkey and signature format
- [libs/db] [\#2293](https://github.com/evdatsion/aphelion-dpos-bft/issues/2293) Allow passing options through when creating instances of leveldb dbs

IMPROVEMENTS:

- [docs] Lint documentation with `write-good` and `stop-words`.
- [docs] [\#2249](https://github.com/evdatsion/aphelion-dpos-bft/issues/2249) Refactor, deduplicate, and improve the ABCI docs and spec (with thanks to @ttmc).
- [scripts] [\#2196](https://github.com/evdatsion/aphelion-dpos-bft/issues/2196) Added json2wal tool, which is supposed to help our users restore (@bradyjoestar)
  corrupted WAL files and compose test WAL files (@bradyjoestar)
- [mempool] [\#2234](https://github.com/evdatsion/aphelion-dpos-bft/issues/2234) Now stores txs by hash inside of the cache, to mitigate memory leakage
- [mempool] [\#2166](https://github.com/evdatsion/aphelion-dpos-bft/issues/2166) Set explicit capacity for map when updating txs (@bluele)

BUG FIXES:

- [config] [\#2284](https://github.com/evdatsion/aphelion-dpos-bft/issues/2284) Replace `db_path` with `db_dir` from automatically generated configuration files.
- [mempool] [\#2188](https://github.com/evdatsion/aphelion-dpos-bft/issues/2188) Fix OOM issue from cache map and list getting out of sync
- [state] [\#2051](https://github.com/evdatsion/aphelion-dpos-bft/issues/2051) KV store index supports searching by `tx.height` (@ackratos)
- [rpc] [\#2327](https://github.com/evdatsion/aphelion-dpos-bft/issues/2327) `/dial_peers` does not try to dial existing peers
- [node] [\#2323](https://github.com/evdatsion/aphelion-dpos-bft/issues/2323) Filter empty strings from config lists (@james-ray)
- [abci/client] [\#2236](https://github.com/evdatsion/aphelion-dpos-bft/issues/2236) Fix closing GRPC connection (@bradyjoestar)
...
- [libs/autofile] [\#2261](https://github.com/evdatsion/aphelion-dpos-bft/issues/2261) Fix log rotation so it actually happens.
  - Fixes issues with consensus WAL growing unbounded ala [\#2259](https://github.com/evdatsion/aphelion-dpos-bft/issues/2259)
...
- [crypto] Switch crypto.Signature from interface to []byte for space efficiency
  [#2128](https://github.com/evdatsion/aphelion-dpos-bft/pull/2128)
...
- [p2p] Remove salsa and ripemd primitives, in favor of using chacha as a stream cipher, and hkdf [#2054](https://github.com/evdatsion/aphelion-dpos-bft/pull/2054)
...
- [blockchain] Improve fast-sync logic
  [#1805](https://github.com/evdatsion/aphelion-dpos-bft/pull/1805)
  - tweak params
...
- [p2p] begin connecting to peers as soon a seed node provides them to you ([#2093](https://github.com/evdatsion/aphelion-dpos-bft/issues/2093))

BUG FIXES:

- [common] Safely handle cases where atomic write files already exist [#2109](https://github.com/evdatsion/aphelion-dpos-bft/issues/2109)
- [privval] fix a deadline for accepting new connections in socket private
  validator.
- [p2p] Allow startup if a configured seed node's IP can't be resolved ([#1716](https://github.com/evdatsion/aphelion-dpos-bft/issues/1716))
- [node] Fully exit when CTRL-C is pressed even if consensus state panics [#2072](https://github.com/evdatsion/aphelion-dpos-bft/issues/2072)

[i1815]: https://github.com/evdatsion/aphelion-dpos-bft/pull/1815
...
- [rpc]: changed the output format for the `/status` endpoint (see https://godoc.org/github.com/evdatsion/aphelion-dpos-bft/rpc/core#Status)
...
- rpc/client: can act as event bus subscriber (See https://github.com/evdatsion/aphelion-dpos-bft/issues/945).
...
- Config and Flags:
  ...
  - The `config` map is replaced with a [`Config` struct](https://github.com/evdatsion/aphelion-dpos-bft/blob/master/config/config.go#L11),
